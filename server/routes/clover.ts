...
import type { CloverStatusResponse, CloverItem, CloverCategory, CloverCatalogResponse } from "@shared/api";
...
export const getCloverStatus: RequestHandler = (_req, res) => {
  ...
  const response: CloverStatusResponse = {
    ...
    missing: [!hasKey ? "CLOVER_API_KEY" : undefined, !hasMerchant ? "CLOVER_MERCHANT_ID" : undefined].filter(Boolean) as string[],
  };
  res.json(response);
};
...
export const listCategories: RequestHandler = async (_req, res) => {
  try {
    ...
    const categories: CloverCategory[] = (data?.elements || []).map((c: any) => ({ id: String(c.id), name: String(c.name || "") }));
    res.json({ categories });
  } catch (e: any) {
...
export const listItems: RequestHandler = async (req, res) => {
  try {
    ...
    const data = await cloverFetch(`/merchants/${merchantId}/items?limit=${limit}&expand=${expand}`);
    const items: CloverItem[] = (data?.elements || []).map((it: any) => ({
      id: String(it.id),
      name: String(it.name || ""),
      price: typeof it.price === "number" ? it.price : null,
      code: it.code ? String(it.code) : undefined,
      category: it.categories?.elements?.[0]?.name ? String(it.categories.elements[0].name) : undefined,
...
async function listItemsPromise() {
  const { merchantId } = ensureEnv();
  if (!merchantId) throw new Error("Missing CLOVER_MERCHANT_ID");
  const data = await cloverFetch(`/merchants/${merchantId}/items?limit=200&expand=categories`);
  const items: CloverItem[] = (data?.elements || []).map((it: any) => ({
    id: String(it.id),
    name: String(it.name || ""),
    price: typeof it.price === "number" ? it.price : null,
