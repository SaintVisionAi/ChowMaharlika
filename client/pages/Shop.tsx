...
export default function Shop() {
  ...
  const categories = useMemo(() => [{ id: "all", name: "All" }, ...(data?.categories || [])], [data]);
  ...
  return (
    <main className="container py-10">
      ...
      {isLoading && <p className="mt-10 text-sm text-foreground/60">Loading catalog…</p>}
      {isError && (
...
function ItemCard({ item }: { item: CloverItem }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const r = await fetch(`/api/clover/item/${encodeURIComponent(item.id)}/images`);
        if (!r.ok) return;
