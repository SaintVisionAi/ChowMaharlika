      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-sm text-foreground/70">
            Live catalog from Clover. Search and filter by category.
          </p>
        </div>
...
function ItemCard({ item }: { item: CloverItem }) {
  const [img, setImg] = useState<string | null>(item.imageUrl || null);

  useEffect(() => {
    let done = false;
    async function load() {
      if (img || !item.id) return;
      try {
        const r = await fetch(
          `/api/clover/item/${encodeURIComponent(item.id)}/images`,
        );
        if (!r.ok) return;
        const data = (await r.json()) as { images: string[] };
        if (!done && data.images?.length) setImg(data.images[0]);
      } catch {}
    }
    load();
    return () => {
  ...
  return (
    <article className="group overflow-hidden rounded-xl border border-border/60 bg-[hsl(var(--charcoal-900))] shadow-xl">
      {img ? (
        ...
        <div className="h-40 w-full bg-[radial-gradient(400px_160px_at_50%_0%,hsl(var(--gold-600))/25%,transparent)] grid place-items-center">
          <span className="text-4xl font-brand text-[hsl(var(--gold-500))]">
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="whitespace-nowrap text-[hsl(var(--gold-400))]">
        ...
        <button
          disabled={!inStock}
          className="mt-3 w-full rounded-md border border-[hsl(var(--gold-600))]/60 bg-gradient-to-b from-[hsl(var(--gold-500))] to-[hsl(var(--gold-600))] px-4 py-2 text-sm font-semibold text-[hsl(var(--charcoal-900))] shadow-glow-gold transition hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed"
        >
