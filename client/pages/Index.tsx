...
export default function Index() {
  ...
  const send = async () => {
    ...
    try {
      const history: ChatMessage[] = messages.map((m) => ({ role: m.from === "user" ? "user" : "assistant", content: m.text }));
      ...
      setMessages((m) => [...m, { from: "saintchow", text: reply }]);
    } catch (e: any) {
      setMessages((m) => [
        ...m,
        {
          from: "saintchow",
          text: "Assistant not available yet. Add OPENAI_API_KEY to enable chat.",
        },
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    <main className="bg-background text-foreground">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="container grid min-h-[72vh] items-center gap-10 py-16 md:grid-cols-2">
          <div>
            ...
            <h1 className="mt-4 font-brand text-5xl leading-tight text-foreground md:text-6xl">Maharlika Seafood Mart & Chow</h1>
            <p className="mt-4 max-w-xl text-foreground/70">Premium seafood, Asian groceries, and hot meals with effortless checkout, rewards, and delivery. Deep charcoal with golden accents for a refined experience.</p>
            <div className="mt-8 flex flex-wrap gap-3">
              <a href="/shop" className="inline-block"><GoldButton>Shop now</GoldButton></a>
              <a href="/delivery" className="inline-block"><button className="inline-flex items-center justify-center gap-2 rounded-md border border-border/60 bg-[hsl(var(--charcoal-800))] px-6 py-3 text-sm font-semibold text-foreground/90 hover:text-foreground">Delivery options</button></a>
            </div>
          ...
          <div className="rounded-xl border border-border/70 bg-[hsl(var(--charcoal-850))] p-4 shadow-xl">
            <div className="flex items-center gap-2 border-b border-border/60 pb-3">
              <div className="inline-flex h-9 w-9 items-center justify-center rounded-md bg-gradient-to-b from-[hsl(var(--gold-500))] to-[hsl(var(--gold-700))] text-[hsl(var(--charcoal-900))]"><Bot className="h-5 w-5" /></div>
              <div><div className="font-semibold">SaintChow Assistant</div><div className="text-xs text-foreground/60">Ask about inventory, orders, or service.</div></div>
            </div>
            ...
            <div className="mt-3 flex gap-2">
              <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder="Ask about salmon, rewards, or hours" className="flex-1 rounded-md border border-border/60 bg-[hsl(var(--charcoal-900))] px-3 py-2 text-sm outline-none placeholder:text-foreground/40" />
              <GoldButton onClick={send} disabled={loading}>{loading ? "Sending…" : "Send"}</GoldButton>
            </div>
            <p className="mt-2 text-xs text-foreground/50">Live chat connects to OpenAI via a secure server proxy when configured.</p>
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-border/60 py-20">
        <SectionTitle eyebrow="Built for speed" title="Seamless shopping in‑store and online" desc="One login across Clover terminals and the web. Delivery partners supported. Rewards that customers love." />
      ...
      <section className="relative overflow-hidden border-t border-border/60 py-20">
        ...
        <div className="container flex flex-col items-center text-center">
          ...
          <p className="mt-3 max-w-2xl text-foreground/70">Explore our fresh catch and house specials. Order pickup, schedule delivery, and earn rewards every time.</p>
...
function Feature({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
  return (
    <div className="rounded-xl border border-border/70 bg-[hsl(var(--charcoal-900))] p-6 shadow-xl">
      <div className="inline-flex h-10 w-10 items-center justify-center rounded-md bg-[hsl(var(--charcoal-800))] text-[hsl(var(--gold-400))]">{icon}</div>
